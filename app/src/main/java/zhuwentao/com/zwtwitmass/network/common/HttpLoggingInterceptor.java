...
public final class HttpLoggingInterceptor implements Interceptor {
	...
	public interface Logger {
		...
		/**
		 * A {@link Logger} defaults output appropriate for the current
		 * platform.
		 */
	...
	public Response intercept(Chain chain) throws IOException {
		...
		if (!logHeaders && hasRequestBody) {
			requestStartMessage += " (" + requestBody.contentLength()
					+ "-byte body)";
		}
		...
		String bodySize = contentLength != -1 ? contentLength + "-byte"
				: "unknown-length";
		...
		if (logHeaders) {
			...
			if (!logBody || !HttpHeaders.hasBody(response)) {
				...
				if (!isPlaintext(buffer)) {
					logger.log("");
					logger.log("<-- END HTTP (binary " + buffer.size()
							+ "-byte body omitted)");
					return response;
				}
